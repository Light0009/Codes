...
public class joinstrings{
    public static void main(String[] args) throws IOException{
        ...
        int n=io.getInt();
        HashMap<Integer,String> wordstorage=new HashMap<Integer,String>();
        for (int i=1;i<=n;i++){
            wordstorage.put(i,io.getWord());
        }
        HashMap<Integer,TailedLinkedList> seq=new HashMap<Integer,TailedLinkedList>();

        int first=0;
        if(n==1){
            pw.println(wordstorage.get(1));
        }
        else{
            for (int j=0;j<(n-1);j++){
                int Sa=io.getInt();
                int Sb=io.getInt();
                if(seq.containsKey(Sa)==true && seq.containsKey(Sb)==true){
                    TailedLinkedList list = new TailedLinkedList();
                    TailedLinkedList list2=new TailedLinkedList();
                    list=seq.get(Sa);
                    list2=seq.get(Sb);
                    ListNode a=seq.get(Sa).getTail();
                    list.join(a,list2);
                    seq.replace(Sa,list);
                }
                else if(seq.containsKey(Sa)==false && seq.containsKey(Sb)==true){
                    TailedLinkedList list = new TailedLinkedList();
                    list=seq.get(Sb);
                    list.addFront(Sa);
                    seq.put(Sa,list);
                }
                else if(seq.containsKey(Sa)==true && seq.containsKey(Sb)==false){
                    TailedLinkedList list=new TailedLinkedList();
                    list=seq.get(Sa);
                    list.addBack(Sb);
                    seq.replace(Sa,list);
            ...
            TailedLinkedList word=seq.get(first);
            for (int k=0;k<n;k++){
                pw.print(wordstorage.get(word.removeFront()));
            }
        }
        pw.close();
        io.close();
...
class TailedLinkedList implements ListInterface {
  ...
  public void join(ListNode onet,TailedLinkedList twoh){
    onet.setNext(twoh.getHead());
    tail=twoh.getTail();
    num_nodes+=twoh.size();
  }
